# fmoralesosornio.github.io
**Fernanda Morales / Portafolio**
Mis Proyectos

¡Hola! 👋  
Soy Data Analyst Jr con conocimientos en Excel, SQL y Python, enfocada en análisis de datos, limpieza, visualización y generación de insights para la toma de decisiones.

---

## 👩‍💼 Sobre mí
- 📍 México  
- 📈 Interés en análisis de datos, negocio y optimización de procesos  
- 🛠️ Herramientas: Excel, SQL, Python (Pandas, NumPy), Power BI / Tableau  
- 🎯 Busco oportunidades Jr o prácticas en Data Analytics

---

## 📂 Proyectos
A continuación algunos de los proyectos que he desarrollado:

### 🔹 Proyecto 1: Análisis de Ventas
- **Descripción:** Análisis exploratorio de ventas para identificar patrones y oportunidades.
- **Herramientas:** Excel, Python, Pandas
- 📁 Carpeta: `/Proyecto_Analisis_Ventas`

### 🔹 Proyecto 2: Congestión vs PIB
- **Descripción:** Relación entre congestión vial y PIB per cápita en distintas ciudades.
- **Herramientas:** Python, visualización de datos
- 📁 Carpeta: `/Proyecto_Congestion_PIB`

---

## 📫 Contacto
- **correo:** fmoralesosornio@gmail.com
- **teléfono:** 442-833-9200
- **Linkedn:** www.linkedin.com/in/fernanda-morales-45567b18a
